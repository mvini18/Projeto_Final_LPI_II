<!DOCTYPE html>
<link href="//maxcdn.bootstrapcdn.com/bootstrap/4.1.1/css/bootstrap.min.css" rel="stylesheet" id="bootstrap-css">
<link href="./css/ajuda.css" type="text/css" rel="stylesheet"/>
<script src="//maxcdn.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js"></script>
<script src="//code.jquery.com/jquery-1.11.1.min.js"></script>
<link rel="icon" href="assets/img/lupus_icone.png" />
<!------ Include the above in your HEAD tag ---------->
<title>Lupus</title>
<div class="wrapper fadeInDown">
  <div id="formContent">
    <!-- Icon -->
    <div class="fadeIn first"><br>
      <img src="assets/img/enfermeira.png" />
    </div><br>

	<div class="container">
	  <div class="row">
	    <div class="col fadeIn second"><h5>Mudança de senha</h5></div>
	    <div class="col fadeIn second"><h5>Problemas com o atestado</h5></div>
	    <div class="w-100"></div>
	    <div class="col fadeIn third"><p>Parar criar uma nova senha é necessário confirmar a antiga.</p></div>
	    <div class="col fadeIn third"><p>O atestado só é gerado caso todas as informações forem preenchidas corretamente.</p></div>
	  </div>
	</div>
    
    <div id="formFooter">
      <a href="primeira_pagina.jsp"><img src="assets/img/retorna.png"></a>
    </div>

  </div>
</div>